...
### Phase 5: 動作確認
- [x] 拡張機能をChromeに読み込んで動作確認
- [x] AudioChunk が正しく生成されることを確認
...
### Phase 9: UI/UX 改善
- [x] 録音中もタブ音声を再生（ユーザーが動画を聴けるように）
- [x] カスタム翻訳プロンプト設定（設定タブで変更可能）
- [x] ログ表示機能（ホームタブに投稿済み/バッファ中を色分け表示）
- [x] マイク権限ステータス表示（ホームタブに許可ボタン付き）
- [x] ポップアップサイズ調整（560px）

### 今後の機能追加予定
- [ ] Slack スレッド設定（親メッセージ指定、スレッド内投稿）
- [ ] チャンク秒数設定（現在固定10秒 → カスタマイズ可能に）
- [ ] 話者分離の調査・実装（優先度: 中）
- [ ] エラーハンドリング・リトライ処理
- [ ] E2E テスト（Playwright）
...
### 既知の課題
- Offscreen Document からのマイク権限取得が失敗する場合がある
  - 「Permission dismissed」エラー
  - タブ音声のみでも動作は継続する
  - Popup でマイク権限ステータスを表示し、許可ボタンを設置済み
